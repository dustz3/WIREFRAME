...
function renderStatus(statusData) {
  ...
  const statusItems = [
    { label: '目前狀態', value: currentStatus || '—' },
    { label: '最後更新', value: updatedAt || '—' },
    { label: '目前位置', value: location || '—' },
    { label: '預計抵達', value: eta || '—' },
  ];
...
function renderTimeline(events) {
  ...
  if (!events || events.length === 0) {
    timeline.innerHTML = '<li class="timeline-placeholder">尚未有資料。</li>';
    return;
  }
  ...
  events
    ...
    .forEach((event) => {
      ...
      item.innerHTML = `
        ...
        <div class="timeline-meta">${event.timestamp}｜${event.location}</div>
...
async function handleTracking(trackingNo) {
  ...
  if (result === 'error') {
    ...
    timeline.innerHTML =
      '<li class="timeline-placeholder">稍候再試或聯絡客服人員。</li>';
    return;
  }
